...
def get_prediction(img, threshold):
    ...
    pred_score_ok = pred_score[:pred_t+1]
    print(pred_score_ok)

    return masks, pred_boxes, pred_class, pred_score_ok
...
def instance_segmentation_api(img_path, threshold=0.5, rect_th=2, text_size=1, text_th=2):
    ...
    for i in range(len(masks)):
        ...
        cv2.putText(img, pred_cls[i] + " : " + str(int(pred_score[i]*100)) + "%", pt1, cv2.FONT_HERSHEY_SIMPLEX,
